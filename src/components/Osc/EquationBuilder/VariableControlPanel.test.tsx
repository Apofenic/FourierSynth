...
function TestWrapper({ expression }: { expression?: string }) {
  ...
  }, [expression]); // Only re-run when expression prop changes
...
describe("VariableControlPanel", () => {
  describe("Empty State", () => {
    it("should render empty state when no variables", () => {
      ...
      expect(
        screen.getByText(/Variables are single letters/i)
      ).toBeInTheDocument();
    });
    ...
    it('should disable "Reset All" button when no variables', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.queryByText(/No variables detected/i)
        ).toBeInTheDocument();
      });

      // The button should not be rendered in empty state
      const resetAllButton = screen.queryByRole("button", {
        name: /Reset All/i,
      });
  ...
  describe("Configuration Dialog", () => {
    ...
    it("should close dialog when cancel clicked", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.queryByText(/Configure Variable: a/i)
